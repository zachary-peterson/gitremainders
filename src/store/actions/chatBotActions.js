...
export const chatFunctionality = (msg) => (dispatch) => {
    ...
    const languageSetting = dialogueOptions.languageReducer.language;
    
    const jokeOptions = (Math.floor(Math.random() * Math.floor(3)) + 1);
    ...
    setTimeout(() => {
        ...
        if(msg.message.includes("greeting")) {
            ...
            if((msg.message.includes("favorite") && msg.message.includes("color")) || (msg.message.includes("set background") && msg.message.includes("teal"))) {
                ...
                dispatch({ type: SET_BACKGROUND, payload: "3c6e71" });
            ...
            dispatch({ type: CHAT, payload: {message: dialogueOptions.languageReducer[dialogueOptions.languageReducer.language].jokes['j'+jokeOptions].p1 }})
            dispatch({ type: CHAT, payload: {message: dialogueOptions.languageReducer[dialogueOptions.languageReducer.language].jokes['j'+jokeOptions].p2 }})
